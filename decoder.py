...
class Decoder(nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        self.T = torch.linspace(0., 1., self.args.decoder_interval + 1) * self.args.decoder_scale
    ...
    def forward(self, y0, graph):
        ...
        ret = y0
        out_ret = []
        for idx in range(self.args.seq_out):
            self.ode_func.set_graph(graph)
            ret = self.ode_dynamics(self.T, ret)[-1]
            out_ret.append(ret)
        out_ret = torch.stack(out_ret)
        out_ret = self.output_layer(out_ret)
        return out_ret.transpose(0, 1)

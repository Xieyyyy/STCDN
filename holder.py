...
class Holder():
    ...
    def train(self, inputs, reals):
        ...
        continous_out, decrete_out, z_mu, z_sigma = self.model(inputs)
        # print(continous_outputs.shape)
        reals = reals[:, :self.args.seq_out, :, :]
        prediction = self.args.scaler.inv_transform(decrete_out)
    ...
    def eval(self, inputs, reals):
        ...
        self.model.eval()
        with torch.no_grad():
            continous_out, decrete_out, _, _ = self.model(inputs)
